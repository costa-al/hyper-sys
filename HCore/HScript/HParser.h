...
protected:
	...
	HTArray<FParsePos>	m_SavedPos;
	HTArray<INT>		m_SkipStack;
	HBOOL				m_bNotSkip;

	HBOOL IsDefine(FString Macros);

public:
	HTBuffer<TCHAR>		*m_pCurBuf;
	TCHAR	**m_pSintax;
	HTMultiMap<FString, HTBuffer<TCHAR>*> m_Buffers;
	...
	const TCHAR *ParseError( const TCHAR *Format, ... );
	void SetBuffer( HTBuffer<TCHAR>* pBuffer );
	void GetToken( const TCHAR* pBreakStr = NULL );
	void ParseToken();
	void ExpectToken( const TCHAR* pString );
	HBOOL CheckToken( const TCHAR* pString );
	void ParseName( FString &Name );
